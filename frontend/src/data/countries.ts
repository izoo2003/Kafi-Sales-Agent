...
export const COUNTRIES: Country[] = COUNTRY_DATA.map((country) => ({
  ...country,
  flag: countryFlag(country.code),
}));

const BY_NAME = new Map(COUNTRIES.map((country) => [country.name.toLowerCase(), country]));
const BY_CODE = new Map(COUNTRIES.map((country) => [country.code.toLowerCase(), country]));
...
export function findCountry(value: string | null | undefined): Country | null {
  ...
  if (BY_CODE.has(key)) return BY_CODE.get(key)!;
  for (const country of COUNTRIES) {
    if (key.includes(country.name.toLowerCase())) return country;
  }
