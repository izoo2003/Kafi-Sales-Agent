...
# and can hang cold starts past Railway's proxy timeout (browser sees CORS).
set -eu
cd "$(dirname "$0")"
python -c "from db.migrate import run_migrations; run_migrations()"
export KAFI_SKIP_LIFESPAN_MIGRATE=1
# 2 workers: enough concurrency without OOM / DB pool exhaustion on typical Railway plans.
